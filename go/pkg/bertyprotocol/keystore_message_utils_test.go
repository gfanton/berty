...
func Test_EncryptMessagePayload(t *testing.T) {
	...
	err = mkh2.RegisterChainKey(ctx, g, omd1.device.GetPublic(), ds1, false)
	assert.NoError(t, err)
	...
	err = mkh1.deriveDeviceSecret(ctx, g, omd1.device)
	assert.NoError(t, err)
	...
	payloadClr1, decryptInfo, err := mkh2.openPayload(ctx, cid.Undef, payloadEnc1, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+2))
	assert.Error(t, err)
	assert.Nil(t, decryptInfo)
	assert.Equal(t, "", string(payloadClr1))

	// Test with a valid counter value, but no CID (so no cache)
	payloadClr1, decryptInfo, err = mkh2.openPayload(ctx, cid.Undef, payloadEnc1, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+1))
	assert.NoError(t, err)
	assert.Equal(t, string(payloadRef1), string(payloadClr1))

	err = mkh2.postDecryptActions(ctx, decryptInfo, g, omd2.device.GetPublic(), mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+1))
	assert.NoError(t, err)
	...
	payloadClr2, decryptInfo, err := mkh2.openPayload(ctx, cid.Undef, payloadEnc2, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+2))
	assert.NoError(t, err)

	err = mkh2.postDecryptActions(ctx, decryptInfo, g, omd2.device.GetPublic(), mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+2))
	assert.NoError(t, err)

	assert.Equal(t, string(payloadRef1), string(payloadClr2))

	// Make sure that a message without a CID can't be decrypted twice
	payloadClr2, decryptInfo, err = mkh2.openPayload(ctx, cid.Undef, payloadEnc2, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+1))
	assert.Error(t, err)
	...
	payloadClr3, decryptInfo, err := mkh2.openPayload(ctx, dummyCID1, payloadEnc3, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+2))
	assert.Error(t, err)
	assert.Equal(t, "", string(payloadClr3))

	payloadClr3, decryptInfo, err = mkh2.openPayload(ctx, dummyCID1, payloadEnc3, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+3))
	assert.NoError(t, err)
	assert.Equal(t, string(payloadRef2), string(payloadClr3))

	err = mkh2.postDecryptActions(ctx, decryptInfo, g, omd2.device.GetPublic(), mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+3))
	assert.NoError(t, err)
	...
	// Wrong CID
	payloadClr3, decryptInfo, err = mkh2.openPayload(ctx, dummyCID2, payloadEnc3, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+3))
	assert.Error(t, err)
	assert.Equal(t, "", string(payloadClr3))

	// Reused CID, wrong counter value
	payloadClr3, decryptInfo, err = mkh2.openPayload(ctx, dummyCID1, payloadEnc3, mustMessageHeaders(t, omd1.device.GetPublic(), initialCounter+4))
	assert.Error(t, err)
	assert.Equal(t, "", string(payloadClr3))
	...
	for i := uint64(0); i < massExpected; i++ {
		...
		payloadClr, decryptInfo, err := mkh2.openPayload(ctx, cid.Undef, payloadEnc, mustMessageHeaders(t, omd1.device.GetPublic(), counter))
		if !assert.NoError(t, err) {
			t.Fatalf("failed at i = %d", i)
		}

		err = mkh2.postDecryptActions(ctx, decryptInfo, g, omd2.device.GetPublic(), mustMessageHeaders(t, omd1.device.GetPublic(), counter))
		assert.NoError(t, err)
...
func Test_EncryptMessageEnvelope(t *testing.T) {
	...
	err = mkh2.RegisterChainKey(ctx, g, omd2.device.GetPublic(), ds2, true)
	assert.NoError(t, err)
...
func Test_EncryptMessageEnvelopeAndDerive(t *testing.T) {
	...
	err = mkh1.RegisterChainKey(ctx, g, omd1.device.GetPublic(), ds1, true)
	assert.NoError(t, err)
